...
(function () {
    ...
    function get_videourl() {
        ...
        try {
            ...
            //狐狸
            else if(location.href.match("/mobile/videoInfo") != null){
                console.log("this")
                player = document.querySelector("#videoBox");
                console.log(player)
                if(true){
                    var videoWrap = document.querySelector("div.videoPlayMainBox").parentNode;
                    videoWrap.style.display = "block"
                    console.log(videoWrap);
                    var bgUrl = videoWrap.nextSibling.style.backgroundImage;
                    videoUrl = `https://xiaoo.cn.jishangsw.xyz${bgUrl.match(/xyz(\S*)1.jpg/)[1]}index.m3u8`; console.log("直播地址:", videoUrl);
                    player.src = videoUrl;
                    var mydiv = document.createElement('div');
                    mydiv.innerHTML = `<p id="my_add_dizhi" style="color:red;font-size:14px">提示：彦祖们，请点击图片下方的链接看破解后的直播视频！${videoUrl}</p>`;
                    videoWrap.after(mydiv);
                    clearInterval(my_timer);
